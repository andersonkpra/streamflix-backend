...
export async function createFavorite(req: Request, res: Response) {
  try {
    ...
    const { movieId, movie, note } = (req.body || {}) as {
      movieId?: string;
...
export async function deleteFavorite(req: Request, res: Response) {
  try {
    const userId = (req as any).userId;
    const idOrMovieId = req.params.id;

    let favorite = null;

    // Primero intentamos eliminar por ObjectId
    if (/^[0-9a-fA-F]{24}$/.test(idOrMovieId)) {
      favorite = await Favorite.findOneAndDelete({ _id: idOrMovieId, userId }).exec();
    }

    // Si no era un ObjectId válido, probamos por movieId
    if (!favorite) {
      favorite = await Favorite.findOneAndDelete({ movieId: idOrMovieId, userId }).exec();
    }
    ...
    return res.json({ message: "Deleted", favorite });
  } catch (err: any) {
